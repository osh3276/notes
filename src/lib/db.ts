...
export async function testConnections(): Promise<{ dbStatus: string, geminiStatus: string }> {
  try {
    // Test database connection
    const dbResult = await sql`SELECT 1 as test`;
    const dbStatus = dbResult ? "Database connection successful" : "Database connection failed";
...
export async function getSongReviewsSummary(songId: string): Promise<string> {
  ...
  try {
    ...
    if (result.length < 1) { // set to 1 for testing
      return '';
    }

    // Combine all reviews into one text, including ratings
    const allReviews = result.map(row => `Rating: ${row.rating}/5\nReview: ${row.review}`).join('\n\n');
